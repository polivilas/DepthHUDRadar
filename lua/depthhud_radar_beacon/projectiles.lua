...
BEACON.projectiles.npc_grenade_frag = {
	...
	Color(255,0,0,128),
	function(ent)
		return BEACON.spriteScale * (1 + (RealTime()*3) % 1)
	end,
	true
}
...
BEACON.projectiles.hunter_flechette = {
	...
	function(ent)
		if ent:GetVelocity():Length() < 16 then
			return BEACON.spriteScale * (1 + (RealTime()*3) % 1) * 0.7
		end
		return BEACON.spriteScale
	end,
	true
...
BEACON.projectiles.prop_combine_ball = {
	...
	Color(255,255,0,128),
	function(ent)
		return BEACON.spriteScale * (1 + math.cos(math.rad(RealTime()*360*2))*0.3 + 0.3)
	end,
	false
}
